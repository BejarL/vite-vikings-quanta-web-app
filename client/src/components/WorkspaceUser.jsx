...
const WorkspaceUser = ({ user, getUsers }) => {
  ...
  return (
    <>
      <tr className="w-[100%]">
        <td className="border-t border-gray-300 p-2 max-w-[100px] overflow-x-auto">
          {user.email}{" "}
        </td>
        ...
        <td className="border-l border-t border-gray-300 p-2">
          {role === "Creator" ? (
            <p>Creator</p>
          ) : role === "Admin" && workspace.workspace_role === "Admin" ? (
            <p>Admin</p>
          ) : (
            <select
        ...
        <td className="border-l border-t border-gray-300 p-2">
          ...
          {role === "Creator" ? (
            <p className="text-gray-500 p-2">Owner</p>
          ) : role === "Admin" && workspace.workspace_role === "Admin" ? (
            <p className="text-gray-500 p-2">Remove</p>
          ) : (
            <>
              <button
                className="hover:bg-red-200 hover:text-red-500 text-black p-2 py-2 rounded-md mr-1 md:order-3"
                onClick={toggleModal}
              >
                Remove
              </button>
